...
// функция для записи строкового снилса
// в числовой снилс в обратном порядке, для удобства вычислений
...
int main(void) {
  ...
  int const SIZE_SNILS_NUMBERS = 11; // yyxxxxxxxxx

  int arr[SIZE_SNILS_NUMBERS] = {0};
  char *pstr = str;
  int *parr = arr + SIZE_SNILS_NUMBERS - 1;
  str_snils_to_array(pstr, parr);

  int control_number = arr[1] * 10 + arr[0];
